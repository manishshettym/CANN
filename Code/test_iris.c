...
void load_data() 
{
    ...
    FILE *in = fopen("datasets/iris.data", "r");
    if (!in) {
...
{
    printf("Training CANN... IRIS dataset\n");
    ...
    for (i = 0; i < loops; ++i) 
    {
        ...
        cann_train(ann, input + j*4, class + j*3, .01);

	    double *op = cann_run(ann, input + j*4);
	    total_loss += loss(op,class + j*3,3);
    ...
    for (j = 0; j < samples; ++j) 
    {
        const double *guess = cann_run(ann, input + j*4);
